...
class Interface(wx.Frame):
    ...
    def coletarDados(Self, v):
        ...
        Self.Texto_estatico_preco = wx.StaticText(dialogo, label="Digite o preço total do produto no formato real.Centavo:")
        Self.formula_preco = wx.TextCtrl(dialogo, -1, name="Digite o preço total do produto no formato real.centavo:")
        Self.texto_estatico_quantidade = wx.StaticText(dialogo, label="Digite a quantidade do produto:")
        Self.formula_quantidade = IntCtrl(dialogo, -1, name="Digite a quantidade do protudo:")
        Self.texto_estatico_porcentagem = wx.StaticText(dialogo, label="Digite a porcentagem que será a base para o seu lucro:")
        Self.formula_porcentagem = IntCtrl(dialogo, -1, name="Digite a porcentagem que será a base para o seu lucro:")
        botao_adicionar_produto = wx.Button(dialogo, wx.ID_OK, label="Adicionar produto")
        botao_cancelar = wx.Button(dialogo, wx.ID_CANCEL, label="Cancelar a adição deste produto")
        resposta = dialogo.ShowModal()
        if resposta == wx.ID_OK:
            ...
            try:
                ...
                informacoes_produto = 'Produto adicionado com sucesso!\nVocê deve vendê-lo a %2s.\ncada unidade deve ser vendida a %2s.\nO lucro por unidade será de %2s, totalizando o lucro de %2s.\nPara mais informações, por favor consulte o arquivo "relatório.txt"' % (resultado_calculo["preco_venda"], resultado_calculo["preco_venda_unidade"], resultado_calculo["lucro_unidade"], resultado_calculo["lucro_total"])
